...
- tldr: 用2-3句话概括论文的核心贡献和意义（中文，共100字以内）
- background: 该研究要解决什么问题、动机是什么（中文，150字以内）
- method: 核心方法或关键思路的简要说明，避免技术细节，用通俗语言解释（中文，200字以内）
- key_findings: 数组，3-5条最重要的实验发现或结论（中文，每条50字以内）
...
返回JSON：
- problem: 论文要解决的问题及其背景动机（中文，150字以内）
- contributions: 数组，2-4条核心贡献（中文，每条60字以内）
- novelty: 与已有工作相比的创新之处（中文，100字以内）
- significance: 学术价值和应用前景评估（中文，80字以内）
...
返回JSON：
- approach: 技术路线整体概述（中文，150字以内）
- key_techniques: 数组，3-5个关键技术点或组件（中文，每条50字以内）
- innovations: 方法层面的创新之处（中文，100字以内）
- pipeline: 方法流程简述，从输入到输出的步骤（中文，100字以内）
...
返回JSON：
- datasets: 数据集信息（中文，100字以内）
- baselines: 对比的基准方法（中文，100字以内）
- main_results: 数组，3-5条主要实验结果（中文，每条50字以内）
- ablation: 消融实验的关键发现（中文，80字以内，如论文无消融实验则写"无"）
- key_findings: 数组，3-5条最重要的实验发现或结论（中文，每条50字以内）
...
返回JSON：
- strengths: 数组，2-3条主要优势（中文，每条50字以内）
- limitations: 数组，2-3条局限性或不足（中文，每条50字以内）
- assumptions: 关键假设条件及其合理性简评（中文，80字以内）
- future_work: 可能的改进方向或后续研究建议（中文，80字以内）
...
- tldr 综合 core_contribution 和 experiment_analysis 的核心发现
- background 使用 core_contribution 的 problem 字段
- method 使用 method_analysis 的 approach，融入 key_techniques
- key_findings 综合 experiment_analysis 和 core_contribution
- entities 综合 method_analysis.entities，加上 authors 作为"人物"类型实体，去重
...
- abstract: 摘要
- keywords: 数组
- tldr: 2-3句话概括核心贡献和意义（中文，100字以内）
- background: 研究背景和动机（中文，150字以内）
- method: 核心方法说明（中文，200字以内）
- key_findings: 数组，3-5条关键发现（中文，每条50字以内）
